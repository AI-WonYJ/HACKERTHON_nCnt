...
def sql_recorder(Now_Time, Now_people):
	conn = pymysql.connect(host="127.0.0.1", user="root", password="", db="nCntDB", charset="utf8")
	cur=conn.cursor()
	# cur.execute("CREATE  TABLE numberTable (time char(30), people_num INT)")
	cur.execute("INSERT INTO numberTable VALUES('{0}', '{1}')".format(Now_Time, Now_people))
	conn.commit()
	conn.close()
...
while True:
  ...
  if time != old_time2 :
    old_time2 = time
    sql_recorder(time, data['counting'])
    print(time, data['counting'])
